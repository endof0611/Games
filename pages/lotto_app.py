...
# --- [DBA 관리 영역] 당첨 번호 히스토리 데이터 배열 ---
# 최신 회차가 추가될 때마다 이 리스트에 [회차, 날짜, 1, 2, 3, 4, 5, 6, 보너스]를 추가하면 됩니다.
LOTTO_HISTORY_DATA = [
    ...
    [1070, "2023-06-03",  1, 2, 11, 21, 30, 35, 39],  
    [1071, "2023-06-10",  3, 6, 14, 22, 30, 41, 36],  
    ...
    [1151, "2024-12-21",  2, 3, 9, 15, 27, 29, 8],  
    [1152, "2024-12-28",  30, 31, 32, 35, 36, 37, 5],  
    [1153, "2025-01-04",  1, 9, 10, 13, 35, 44, 5],  
    [1154, "2025-01-11",  4, 8, 22, 26, 32, 38, 27],  
    [1155, "2025-01-18",  10, 16, 19, 27, 37, 38, 13],  
    [1156, "2025-01-25",  30, 31, 34, 39, 41, 45, 7],  
    [1157, "2025-02-01",  5, 7, 12, 20, 25, 26, 28],  
    [1158, "2025-02-08",  21, 25, 27, 32, 37, 38, 20],  
    [1159, "2025-02-15",  3, 9, 27, 28, 38, 39, 7],  
    [1160, "2025-02-22",  7, 13, 18, 36, 39, 45, 19],  
    [1161, "2025-03-01",  2, 12, 20, 24, 34, 42, 37],  
    [1162, "2025-03-08",  20, 21, 22, 25, 28, 29, 6],  
    [1163, "2025-03-15",  2, 13, 15, 16, 33, 43, 4],  
    [1164, "2025-03-22",  17, 18, 23, 25, 38, 39, 22],  
    [1165, "2025-03-29",  6, 7, 27, 29, 38, 45, 17],  
    [1166, "2025-04-05",  14, 23, 25, 27, 29, 42, 16],  
    [1167, "2025-04-12",  8, 23, 31, 35, 39, 40, 24],  
    [1168, "2025-04-19",  9, 21, 24, 30, 33, 37, 29],  
    [1169, "2025-04-26",  5, 12, 24, 26, 39, 42, 20],  
    [1170, "2025-05-03",  3, 13, 28, 34, 38, 42, 25],  
    [1171, "2025-05-10",  3, 6, 7, 11, 12, 17, 19],  
    [1172, "2025-05-17",  7, 9, 24, 40, 42, 44, 45],  
    [1173, "2025-05-24",  1, 5, 18, 20, 30, 35, 3],  
    [1174, "2025-05-31",  8, 11, 14, 17, 36, 39, 22],  
    [1175, "2025-06-07",  3, 4, 6, 8, 32, 42, 31],  
    [1176, "2025-06-14",  7, 9, 11, 21, 30, 35, 29],  
    [1177, "2025-06-21",  3, 7, 15, 16, 19, 43, 21],  
    [1178, "2025-06-28",  5, 6, 11, 27, 43, 44, 17],  
    [1179, "2025-07-05",  3, 16, 18, 24, 40, 44, 21],  
    [1180, "2025-07-12",  6, 12, 18, 37, 40, 41, 3],  
    [1181, "2025-07-19",  8, 10, 14, 20, 33, 41, 28],  
    [1182, "2025-07-26",  1, 13, 21, 25, 28, 31, 22],  
    [1183, "2025-08-02",  4, 15, 17, 23, 27, 36, 31],  
    [1184, "2025-08-09",  14, 16, 23, 25, 31, 37, 42],  
    [1185, "2025-08-16",  6, 17, 22, 28, 29, 32, 38],  
    [1186, "2025-08-23",  2, 8, 13, 16, 23, 28, 35],  
    [1187, "2025-08-30",  5, 13, 26, 29, 37, 40, 42],  
    [1188, "2025-09-06",  3, 4, 12, 19, 22, 27, 9],  
    [1189, "2025-09-13",  9, 19, 29, 35, 37, 38, 31],  
    [1190, "2025-09-20",  7, 9, 19, 23, 26, 45, 33],  
    [1191, "2025-09-27",  1, 4, 11, 12, 20, 41, 2],  
    [1192, "2025-10-04",  10, 16, 23, 36, 39, 40, 11],  
    [1193, "2025-10-11",  6, 9, 16, 19, 24, 28, 17],  
    [1194, "2025-10-18",  3, 13, 15, 24, 33, 37, 2],  
    [1195, "2025-10-25",  3, 15, 27, 33, 34, 36, 37],  
    [1196, "2025-11-01",  8, 12, 15, 29, 40, 45, 14],  
    [1197, "2025-11-08",  1, 5, 7, 26, 28, 43, 30],  
    [1198, "2025-11-15",  26, 30, 33, 38, 39, 41, 21],  
    [1199, "2025-11-22",  16, 24, 25, 30, 31, 32, 7],  
    [1200, "2025-11-29",  1, 2, 4, 16, 20, 32, 45],  
    [1201, "2025-12-06",  7, 9, 24, 27, 35, 36, 37],  
    [1202, "2025-12-13",  5, 12, 21, 33, 37, 40, 7],  
    [1203, "2025-12-20",  3, 6, 18, 29, 35, 39, 24],  
    [1204, "2025-12-27",  8, 16, 28, 30, 31, 44, 27],  
    ...
    # ... (데이터 압축을 위해 이전 데이터는 생략 가능하며, 여기에 전체 데이터를 paste 하시면 됩니다)
    # [회차, 날짜, 1번, 2번, 3번, 4번, 5번, 6번, 보너스]
]

# --- 데이터 전처리 ---
df = pd.DataFrame(LOTTO_HISTORY_DATA, columns=['회차', '날짜', '1번', '2번', '3번', '4번', '5번', '6번', '보너스'])
df = df.sort_values('회차')

# --- 데이터 분석 엔진 ---
class StrategicLottoAgent:
    def __init__(self, birth_date, history_df):
        self.birth_date = birth_date
        self.df = history_df
        self.all_nums = self.df[['1번', '2번', '3번', '4번', '5번', '6번']].values.flatten().astype(int)

    def get_seed(self, label, date_str):
        hash_val = hashlib.sha256(f"{label}_{date_str}_{self.birth_date}".encode()).hexdigest()
        return int(hash_val[:8], 16)

    def run_simulation(self, model_id, weight_step, date_str):
        seed = self.get_seed(f"{model_id}_{weight_step}", date_str)
        random.seed(seed); np.random.seed(seed)
        
        if model_id.startswith("M"):
            m_idx = int(model_id[1:3])
            current_r = 3.5 + (m_idx * 0.015) + (weight_step * 0.01)
            if current_r >= 4.0: current_r = 3.99
            x = (seed % 1000) / 1000
            pool = []
            for _ in range(40):
                x = current_r * x * (1 - x)
                val = int(x * 45) + 1
                if 1 <= val <= 45: pool.append(val)
            res = list(set(pool))
            return sorted(random.sample(res, 6)) if len(res) >= 6 else sorted(random.sample(range(1, 46), 6))

        elif model_id.startswith("T"):
            t_idx = int(model_id[1:3])
            current_decay = 0.05 * weight_step
            window = 5 + (t_idx * 5)
            # 윈도우 크기가 데이터보다 크면 전체 데이터 사용
            recent_nums = self.all_nums[-(window*6):] if len(self.all_nums) >= (window*6) else self.all_nums
            counts = pd.Series(recent_nums).value_counts().reindex(range(1, 46), fill_value=0).sort_index()
            weights = (counts.max() + 1) - (counts * current_decay)
            weights = weights.clip(lower=0.1)
            probs = weights / weights.sum()
            return sorted(np.random.choice(range(1, 46), size=6, replace=False, p=probs.values).tolist())
        return sorted(random.sample(range(1, 46), 6))

# --- Streamlit UI ---
st.set_page_config(page_title="Lotto DNA Standalone", layout="wide")
st.title("🧬 Lotto DNA Analyzer v3.0 (Standalone)")
st.markdown("CSV 파일 없이 **내장 데이터**로 작동하는 독립형 분석기")

with st.sidebar:
    st.header("⚙️ 분석 설정")
    birth_date = st.text_input("생년월일 (8자리)", value="19770726")
    analysis_window = st.slider("성능 검증 윈도우 (회차)", 5, len(df), min(10, len(df)))
    st.info(f"현재 DB 적재량: {len(df)}개 회차")

tab1, tab2 = st.tabs(["🎯 실전 번호 생성", "📜 데이터베이스 관리"])

agent = StrategicLottoAgent(birth_date, df)

with tab1:
    if st.button("🚀 최적 가중치 분석 및 번호 생성"):
        candidate_models = [f"M{i:02d}" for i in range(1, 11)] + [f"T{i:02d}" for i in range(1, 11)]
        recent_data = df.tail(analysis_window)
        
        with st.spinner('내장 DB 분석 중...'):
            model_stats = []
            for m_id in candidate_models:
                for step in range(1, 7):
                    hits, total_prize = 0, 0
                    for _, row in recent_data.iterrows():
                        actual = {int(row[f'{i}번']) for i in range(1, 7)}
                        bonus = int(row['보너스'])
                        pred = set(agent.run_simulation(m_id, step, row['날짜']))
                        match = len(pred & actual)
                        
                        prize = 0
                        if match >= 3:
                            hits += 1
                            if match == 6: prize = 2000000000
                            elif match == 5 and bonus in pred: prize = 50000000
                            elif match == 5: prize = 1500000
                            elif match == 4: prize = 50000
                            elif match == 3: prize = 5000
                        total_prize += prize
                    
                    win_rate = (hits / analysis_window) * 100
                    roi = (total_prize / (analysis_window * 1000)) * 100
                    model_stats.append({'Model': f"{m_id}_S{step}", 'ROI(%)': roi, 'WinRate(%)': win_rate})

            perf_df = pd.DataFrame(model_stats).sort_values(['ROI(%)', 'WinRate(%)'], ascending=False).head(5)
            
            st.subheader("🏆 최적 가중치 분석 결과")
            st.table(perf_df)

            st.divider()

            st.subheader("🎯 이번 주 실전 추천 조합")
            cols = st.columns(5)
            next_date = datetime.now().strftime("%Y-%m-%d")

            for i, (idx, row) in enumerate(perf_df.iterrows()):
                m_info = row['Model'].split('_')
                m_id, step = m_info[0], int(m_info[1][1:])
                pick = agent.run_simulation(m_id, step, next_date)
                with cols[i]:
                    st.metric(label=f"세트 {i+1}", value=f"{row['ROI(%)']:.0f}%")
                    st.code(f"{pick}", language='python')
            st.success("데이터 무결성 검증 완료.")

with tab2:
    st.subheader("📊 내장 데이터베이스 현황")
    st.dataframe(df.sort_values('회차', ascending=False), use_container_width=True, hide_index=True)
    
    st.info("데이터를 업데이트하려면 코드 내의 'LOTTO_HISTORY_DATA' 배열에 최신 회차를 추가하세요.")